        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Tabungan, progress, dan riwayat transaksi nasabah dalam satu halaman.
            </p>
          </div>
        </div>
      ...
      {p && user && (
        <>
          ...
          <div>
            ...
            {p.produk.tabungan.length === 0 && p.produk.qurban.length === 0 ? (
                    ...
                    <div key={prog.jenis_tabungan_id} className="rounded-3xl bg-white dark:bg-slate-800/90 border border-slate-100 dark:border-slate-800 overflow-hidden">
                      <div className="flex items-center justify-between gap-2 px-5 pt-5">
                        <div className="flex items-center gap-2.5">
                          <div className={`p-2 rounded-xl ${isEmas ? 'bg-amber-100 dark:bg-amber-950 text-amber-600 dark:text-amber-300' : 'bg-sky-100 dark:bg-sky-950 text-sky-600 dark:text-sky-300'}`}>
                            {isEmas ? <Coins className="w-4 h-4" /> : <Wallet className="w-4 h-4" />}
                          </div>
                          <div>
                            <p className="text-sm font-extrabold text-slate-800 dark:text-white">{prog.nama}</p>
                            <p className="text-[10px] text-slate-400 uppercase font-bold">{isEmas ? 'Tabungan Emas' : 'Tabungan Pribadi'}</p>
                          </div>
                        </div>
                        {t.konfigurasi && t.konfigurasi.length > 0 ? (
                          <span className="px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950 text-[10px] font-bold text-amber-700 dark:text-amber-300 flex items-center gap-1 shrink-0">
                            <Repeat className="w-3 h-3" /> Setoran Berkala
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-300 shrink-0">
                            Bebas
                          </span>
                        )}
                      </div>
                      ...
                      {isEmas && (
                        <div className="grid grid-cols-2 text-center divide-x divide-slate-100 dark:divide-slate-800 border-b border-slate-100 dark:border-slate-800">
                          <div className="px-2 py-3">
                            <p className="text-[9px] text-emerald-600 dark:text-emerald-300 font-bold uppercase">Saldo Dana</p>
                            <p className="text-xs font-extrabold text-slate-800 dark:text-white mt-0.5 break-all">Rp {formatRupiah(prog.saldo_dana)}</p>
                          </div>
                          <div className="px-2 py-3">
                            <p className="text-[9px] text-rose-600 dark:text-rose-300 font-bold uppercase">Pending Setor</p>
                            <p className="text-xs font-extrabold text-slate-800 dark:text-white mt-0.5 break-all">Rp {formatRupiah(prog.pending_amount)}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {t.setoran_berkala && t.setoran_berkala.length > 0 && (
                        <div className="px-5 pb-5 pt-4 bg-amber-50/50 dark:bg-amber-950/10 space-y-4">
                            ...
                            <div key={sb.konfigurasi_id ?? 'sb'} className="space-y-3">
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-300">
                                  Rencana {t.setoran_berkala!.length > 1 ? `#${t.setoran_berkala!.indexOf(sb) + 1} ` : ''}• Rp {formatRupiah(sb.nominal_per_periode ?? 0)} {t.progress.tipe === 'emas' ? '·' : ''}
                                </span>
                ...
                {p.produk.qurban.map((q) => (
                  <div key={q.id} className="rounded-3xl bg-white dark:bg-slate-800/90 border border-slate-100 dark:border-slate-800 overflow-hidden">
                    <div className="flex items-center justify-between gap-2 px-5 pt-5">
                      ...
                      <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-300 capitalize shrink-0">
                        {q.status_label}
                      </span>
                    </div>
                    ...
                    <div className="px-5 py-4">
                      ...
                      <p className="text-[10px] text-slate-400 mt-1.5">Terdaftar: {q.tanggal_daftar || '-'}</p>
                    </div>
                  </div>
                ))}
              </div>
